...
/**
 * Заготовка под лимиты запросов — ограничение ГЛУБИНЫ GraphQL-операции.
 *
 * Соцсеть — это циклический граф (user → posts → author → posts → …), поэтому
 * чрезмерно вложенный запрос легко раскрутить в тяжёлую нагрузку. Это базовый
 * щит, который ставим уже сейчас. Полноценные лимиты СЛОЖНОСТИ (стоимость по
 * полям/пагинации) добавим на продакшен-этапе — вот точка их подключения:
 * новая validation-rule рядом с этой и в массиве `validationRules` Apollo.
 *
