...
- **Фон, события и масштабирование:** **событийная развязка** (`@nestjs/event-emitter` — сервисы
  только эмитят доменные события, побочные эффекты в `@OnEvent`-слушателях); **очереди BullMQ**
  (`feed-fanout`/`notifications`/`maintenance`, retry/backoff, `removeOnComplete`); **fan-out
  ленты** (fan-out on write) — публикация поста разносится по лентам подписчиков в Redis sorted
  set, чтение `feed` — O(1) из набора + гибридный бэкофилл из БД; **уведомления** через слушатель
  и очередь доставки с **идемпотентностью** (`SET NX delivered:<id>`); **планировщик**
